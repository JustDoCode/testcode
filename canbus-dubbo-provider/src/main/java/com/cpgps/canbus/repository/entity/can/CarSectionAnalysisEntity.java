 ...
 */
@Entity
@Table(name = "t_canbus_clqjfx")
public class CarSectionAnalysisEntity {
	@Id
	...
	//以下字段为计算数据 不入库
	private double yzsjpt = 0;//运转时间比
	private double xshspt = 0;//行使耗时时间比
	private double xhyhpt = 0;//行使消耗百分比
	private double wsclcpt = 0;//未刹车里程百分比
	//新加字段 不入库
	...
	public double getYzsjpt() {
		return yzsjpt;
	}

	public void setYzsjpt(double yzsjpt) {
		this.yzsjpt = yzsjpt;
	}

	public double getXshspt() {
		return xshspt;
	}

	public void setXshspt(double xshspt) {
		this.xshspt = xshspt;
	}

	public double getXhyhpt() {
		return xhyhpt;
	}

	public void setXhyhpt(double xhyhpt) {
		this.xhyhpt = xhyhpt;
	}

	public double getWsclcpt() {
		return wsclcpt;
	}

	public void setWsclcpt(double wsclcpt) {
		this.wsclcpt = wsclcpt;
	}
